...
public class testusuarios {
    ...
    void testCrearUsuario() {
        Usuario nuevoUsuario = new Usuario("testusu0", "testusu0", "Nuevo Usuario", "testusu@test.com");
    ...
    void testObtenerUsuarioPorIdExistente() {
        Usuario usuario = new Usuario("user1", "pass1", "Usuario Uno", "user1@user1.com");
    ...
    void testObtenerTodosLosUsuarios() {
        Usuario user1 = new Usuario("user1", "pass1", "Usuario Uno", "user1@udc.com");
        Usuario user2 = new Usuario("user2", "pass2", "Usuario Dos", "user2@udc.com");
    ...
    void testActualizarUsuario() {
        Usuario usuarioAActualizar = new Usuario("user1", "pass1", "Usuario Uno", "user1@udc.com");
    ...
    void testObtenerUsuarioPorUsernameExistente() {
        Usuario usuario = new Usuario("user_test", "pass_test", "Usuario Test", "test@udc.com");
    ...
    void testAutenticarUsuarioExitosoPorUsername() {
        Usuario usuario = new Usuario("testuser", "password123", "Test User", "test@udc.com");
    ...
    void testAutenticarUsuarioExitosoPorEmail() {
        Usuario usuario = new Usuario("testuser", "password123", "Test User", "test@udc.com");
        when(usuarioRepository.findByUsername("test@udc.com")).thenReturn(Optional.empty());
        when(usuarioRepository.findByEmail("test@udc.com")).thenReturn(Optional.of(usuario));

        Usuario usuarioAutenticado = usuarioService.autenticarUsuario("test@udc.com", "password123");

        assertNotNull(usuarioAutenticado);
        assertEquals("test@udc.com", usuarioAutenticado.getEmail());
    ...
    void testAutenticarUsuarioFallidoPorCredencialesInvalidas() {
        Usuario usuario = new Usuario("testuser", "password123", "Test User", "test@example.com");
        when(usuarioRepository.findByUsername("testuser")).thenReturn(Optional.of(usuario));
    ...
    void testIniciarRecordarPasswordEncontradoPorUsername() {
        Usuario usuario = new Usuario("testuser", "password", "Test User", "test@udc.com");
        when(usuarioRepository.findByUsername("testuser")).thenReturn(Optional.of(usuario));
        when(usuarioRepository.findByEmail(anyString())).thenReturn(Optional.empty());
    ...
    void testIniciarRecordarPasswordEncontradoPorEmail() {
        ...
        assertNotNull(encontrado);
        assertEquals("test@udc.com", encontrado.getEmail());
        verify(usuarioRepository, times(1)).findByUsername("test@udc.com");
        verify(usuarioRepository, times(1)).findByEmail("test@udc.com");
    }
    ...
    void testSendPasswordRecoveryEmail() {
        Usuario usuario = new Usuario("testuser", "testpass", "Test User", "test@udc.com");
